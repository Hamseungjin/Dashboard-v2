...
import TotalRentalCnt from "../components/monthly/TotalRentalCnt";
// import LineChartContainer from "../components/monthly/LineChartContainer";
    ...
    return (
      <div className="w-full h-screen flex items-center justify-center gap-4">
        ...
        <p className="text-2xl font-semibold">
          {formatData(month)}의 정보를 불러오는 중입니다 . . .
        </p>
      </div>
    );
  }
        ...
        <section className="flex">
          <div className="flex flex-col gap-6 w-[900px]">
            ...
            <div className="w-full flex flex-col justify-between border p-4 py-8 bg-white shadow-lg rounded-lg">
              <div>
                <p className="font-semibold text-xl pb-6 border-b">
                  [ 통계 요약 ]
                </p>
                <p className="pt-8">
                  {formatData(month)} 1000개의 대여소 중 가장 대여 건수가 많았던
                  대여소는 차례대로 {topFiveData[0].name}, {topFiveData[1].name}
                  , {topFiveData[2].name}, {topFiveData[3].name},{" "}
                  {topFiveData[4].name} 입니다.
                </p>
                <p>
                  가장 대여 건수가 많았던 {topFiveData[0].name}에서는 한 달간{" "}
                  {topFiveData[0].use_cnt}번의 따릉이를 대여했으며, 이는{" "}
                  {formatData(month)} 총 대여 건수의{" "}
                  {(topFiveData[0].use_cnt / totalRentCnt).toFixed(2)}%에
                  해당합니다. 두번째로 대여 건수가 많았던 {topFiveData[1].name}
                  에서는 한달간 약 888건의 따릉이를 대여했으며,{" "}
                  {topFiveData[0].name} 대여소와 {topFiveData[1].name} 대여소의
                  한달간 대여 건수는 약{" "}
                  {topFiveData[0].use_cnt - topFiveData[1].use_cnt}건
                  차이납니다.
                </p>
                <a
                  href="http://data.seoul.go.kr/dataList/OA-15248/F/1/datasetView.do"
                  target="_blank"
                >
              ...
              <div className="flex flex-col mt-16 text-sm">
                <p className="text-slate-500">
                  ...
                  '공공자전거이용정보'이며 자료 권한은 서울특별시에 있음을
                  알립니다.
                </p>
